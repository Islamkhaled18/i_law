...
class Books extends Component
{
    ...
    protected $listeners =['ooo'];

    public function render()
    {
        return view('livewire.vendor.books',[
            'books'=>Book::where('id',auth()->user()->id)->orderBy('id','asc')->paginate(5),
    ...
    public function ooo($id){
        $del =  Book::find($id)->delete();
        if($del){
            $this->dispatchBrowserEvent('oood');
        }
    }
}
